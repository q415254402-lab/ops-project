  ...
  var app = new Vue({
    ...
    data: function () {
      return {
        ...
        detailType: 'total',     // total | high | src | dst | ip
        detailLogs: [],
        detailIpList: [],
        detailLoading: false,
        detailTotal: 0,
    ...
    mounted: function () {
      ...
      window.addEventListener('keydown', function (e) {
        if (e.key === 'Escape' && self.drawerLog) {
          self.closeDrawer();
        }
      });
      // 2026-08-18：点击抽屉外部关闭（替代灰色蒙层，mousedown 早于 click 避免 openLog 立即触发关闭）
    ...
    watch: {
      ...
      // 2026-08-17：detail → dash 时图表 DOM 被 v-if 卸载重建，必须 dispose + 重 init（老坑）
      view: function (v) {
        var self = this;
        if (v === 'dash') {
          Object.keys(self.chartInstances).forEach(function (k) {
    ...
    methods: {
      ...
      // 2026-08-17：统计卡下钻（2026-08-18：分页 + 总数，超 500 条也能看全）
      openCard: function (type) {
        ...
        if (type === 'src' || type === 'dst') {
          // 源IP/目标IP → 全量分组统计列表
          fetchJson(API.stats + '?' + self.rangeParams() + '&device_type=WAF')
            .then(function (res) {
              var st = (res && res.data) || {};
          ...
          // total/high → 攻击日志列表（分页：每页 200，可加载更多）
          var q = '?' + self.rangeParams() + '&limit=200&offset=0&device_type=WAF';
      ...
      // 2026-08-18：下钻日志列表加载更多（分页追加）
      loadMore: function () {
        var self = this;
        if (self.detailLoading || !self.detailLogs.length) return;
        self.detailLoading = true;
        var q = '?' + self.rangeParams() + '&limit=200&offset=' + self.detailLogs.length + '&device_type=WAF';
      ...
      // 点 IP 分组 → 显示该 IP 的攻击记录
      filterByIp: function (ip) {
        var self = this;
        var origType = self.detailType;   // src | dst（此时还没改）
        self.detailType = 'ip';
        ...
        fetchJson(API.logs + '?' + self.rangeParams() + '&limit=200&offset=0&device_type=WAF&' + f + '=' + encodeURIComponent(ip))
          .then(function (res) {
            self.detailTotal = ((res && res.data) || {}).total || 0;
            self.detailLogs = ((res && res.data && res.data.data) || []);
            self.detailLoading = false;
      ...
      // 2026-08-17：URL 展示（过滤 "None"/"null" 字符串）
      urlText: function (l) {
        var d = (l && l.domain && l.domain !== 'null' && l.domain !== 'None') ? l.domain : '';
        var u = (l && l.uri && l.uri !== 'null' && l.uri !== 'None') ? l.uri : '';
        return (d + u) || '-';
      },
      openDetail: null,
      ...
      _updateCharts: function () {
        ...
        var self = this;
        // 2026-08-18：图表只用 TOP10（typeDist 可能有十几种攻击类型，柱状图/饼图只画 TOP10）
        ...
        self.chartInstances.src.setOption({
          ...
          // 2026-08-17：grid.left 加大到 110 容纳完整 IP
      ...
      loadAll: function () {
        var self = this;
        Promise.all([
          fetchJson(API.stats + '?' + this.rangeParams() + '&device_type=WAF'),
          fetchJson(API.logs + '?limit=50&device_type=WAF'),
        ]).then(function (res) {
      ...
      exportLogs: function () {
        var q = this.rangeParams() + '&device_type=WAF';
        window.open(API.logsExport + '?' + q, '_blank');
      },
    },
  });

  window.__wafMonitorApp = app;
})();
