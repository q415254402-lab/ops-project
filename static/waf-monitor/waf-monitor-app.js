  ...
  var app = new Vue({
    ...
    data: function () {
      return {
        loading: true,
        range: '24h',
        ranges: [{ v: '24h', t: '近24小时' }, { v: '7d', t: '近7天' }],
    ...
    methods: {
      ...
      openCard: function (type) {
        ...
        if (type === 'src' || type === 'dst') {
          // 源IP/目标IP → 全量分组统计列表
          fetchJson(API.stats + '?range=' + this.range + '&device_type=WAF')
            .then(function (res) {
              var st = (res && res.data) || {};
          ...
          var q = '?limit=200&offset=0&device_type=WAF';
      ...
      loadMore: function () {
        ...
        self.detailLoading = true;
        var q = '?limit=200&offset=' + self.detailLogs.length + '&device_type=WAF';
      ...
      filterByIp: function (ip) {
        ...
        fetchJson(API.logs + '?limit=200&offset=0&device_type=WAF&' + f + '=' + encodeURIComponent(ip))
          .then(function (res) {
            self.detailTotal = ((res && res.data) || {}).total || 0;
            self.detailLogs = ((res && res.data && res.data.data) || []);
            self.detailLoading = false;
